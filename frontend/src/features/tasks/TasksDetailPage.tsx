  ...
  return (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-5 border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Dataset ID: {task.dataset_id}</p>
          </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-1 space-y-4">
            ...
            <h3 className="font-bold text-sm tracking-wide text-muted-foreground uppercase">Task Information</h3>
          ...
          {/* İş Akışı Durum Değiştirme Paneli */}
                    <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4">
            <h3 className="font-bold text-sm tracking-wide text-muted-foreground uppercase">Workflow Actions</h3>
            
            {/* Note Input */}
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">Process Note (Optional)</label>
              <Input 
        ...
        {/* Sağ Panel: Atanan Görseller / Asset Listesi */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-sm tracking-wide text-muted-foreground uppercase">Assigned Assets</h3>
            </div>
                ...
                <div key={img.id} className="flex items-center justify-between p-3 bg-muted/30 hover:bg-muted/60 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono">ID: {img.id}</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card text-card-foreground rounded-xl border border-border w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-4 border-b border-border flex items-center justify-between bg-muted">
              <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Reassign Task Assignee</h3>
            </div>
            <form onSubmit={handleReassignTask} className="p-4 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">Target Username</label>
                <Input 
                  required
                  disabled={isSubmitting}
      ...
      {/* ================= ADD ASSET MODAL ================= */}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card text-card-foreground rounded-xl border border-border w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-4 border-b border-border flex items-center justify-between bg-muted">
              <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Add New Asset to Task</h3>
            </div>
            <form onSubmit={handleAddAsset} className="p-4 space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">Asset UUID / ID</label>
                <Input 
                  required
                  disabled={isSubmitting}
