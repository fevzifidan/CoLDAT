...
// --- 1. ADIM: YAML ŞEMASINA GÖRE TİPLER ---
interface Dataset {
  id: string;               // UUID
  project_id: string;       // YAML: project_id
  name: string;             // YAML: name
  description: string;      // YAML: description
  current_version: string;  // YAML: current_version (e.g., "v1.0")
  total_images: number;     // YAML: total_images
  annotated_images: number; // YAML: annotated_images
  role: 'admin' | 'annotator' | 'viewer'; // YAML: role (current user's role)
}

const DatasetsPage = () => {
  // --- 2. ADIM: YAML'DAN GELEN ÖRNEK VERİLER ---
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto bg-slate-50/50 min-h-screen">
      {/* Header - YAML: Datasets Tag */}
      <div className="flex items-center justify-between border-b pb-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Datasets</h1>
          <p className="text-slate-500 mt-1">Manage versioned data collections and track progress.</p>
        </div>
        <Button className="bg-indigo-600 hover:bg-indigo-700 shadow-md transition-all">
          <Database className="mr-2 h-4 w-4" /> Create New Dataset
        </Button>
      </div>

      {/* Grid - YAML: List Datasets in a Project */}
          ...
          return (
            <Card key={ds.id} className="relative overflow-hidden border-2 hover:border-indigo-200 transition-all group">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-green-50/50 p-3 rounded-lg border border-green-100">
                    <p className="text-[10px] text-green-600/70 uppercase font-bold mb-1">Annotated</p>
              ...
              <CardFooter className="bg-slate-50/80 border-t p-4 flex gap-3 mt-2">
                ...
                {/* YAML: role tabanlı buton görünürlüğü simülasyonu */}
                <Button 
                  size="sm" 
                ...
                >
                  <UserPlus className="mr-1.5 h-3.5 w-3.5" /> Members
                </Button>
              </CardFooter>
            </Card>
          );
