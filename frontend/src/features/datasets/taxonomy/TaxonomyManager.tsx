...
import { Plus, Edit2, Check, X, Link as LinkIcon, Tag, Loader2, Trash2, ShieldCheck } from "lucide-react";
...
// Backend modeline uygun şekilde genişletilmiş Class ve Predicate tipleri
interface ClassItem {
  id: string | number;
  name: string;
  color: string;
...
const TaxonomyManager: React.FC<TaxonomyManagerProps> = ({ projectId, onUpdate }) => {
  ...
  // Ortak düzenleme modunda kullanılacak yeni boolean durumlar
  ...
  useEffect(() => {
    ...
    projectService.getProjectTaxonomy(projectId)
      .then((res: any) => {
        ...
        // Gelen verilerde bu alanlar yoksa varsayılan olarak true atıyoruz
  ...
  const updateParent = (updatedClasses: ClassItem[], updatedPreds: PredicateItem[]) => {
    onUpdate({
      ...
      attributes: [] // Global attributes tamamen temiz kalıyor
    });
  };

  // Düzenlemeyi Kaydetme (Hem Class hem Predicate için ortak logic)
                ...
                <div key={cls.id} className="flex flex-col p-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl group gap-2">
                  {editingId === `class-${cls.id}` ? (
                    ...
                    <div className="space-y-3 w-full">
                      ...
                      {/* Sub-attributes Kontrolleri (isActive / includeInExport) */}
