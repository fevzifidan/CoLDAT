...
/*
REQUIRED COMPANION FRONTEND CHANGES
===================================

1. In types/synthetic.types.ts add Google:

   export type AIProvider = 'google' | 'openai' | 'stability' | 'replicate';

2. In ModelSelector.tsx validate against the selected provider:

   imageGenerationService.validateApiKey(
     value,
     selectedModel?.provider as 'google' | 'openai' | 'stability' | 'replicate'
   )

3. In store/syntheticSlice.ts NEVER persist apiKey. Change partialize to:

   partialize: (state) => ({ selectedModel: state.selectedModel })

   Then remove any UI text claiming that the API key was restored from localStorage.

4. Provider availability, model IDs, billing access, and browser CORS policies
   can change. Test each enabled model with its provider account before release.

5. In public/locales/en/synthetic.json and public/locales/tr/synthetic.json,
   add modelSelector.models.gemini-2.5-flash-image, or render model.name and
   model.description as the i18n fallback. Otherwise the raw translation key
   will appear in the model dropdown.

6. After replacing the old Imagen model, clear the persisted stale selection:

   localStorage.removeItem('synthetic-studio-storage');

7. Saving to a CoLDAT dataset can continue using the existing dataURLtoFile +
   uploadService flow. No backend generation endpoint is required for that step.
*/
